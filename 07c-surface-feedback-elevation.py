...
for e in range(len(elevations) - 1):
    ...
    bulk_t_std.append(np.nanstd(data['surface_feedback'].values[:,:,0][area_valid_t]))
...
ax1.fill_betweenx(elevations[:-1], bulk_t+bulk_t*0.125, bulk_t-bulk_t*0.125,
...
ax3.fill_betweenx(elevations[:-1], coeffs['bulk_gt']+coeffs['bulk_gt']*0.21, 
                  coeffs['bulk_gt']-coeffs['bulk_gt']*0.21,
                  zorder=1, color=c4, alpha=0.2)
...
ax4.fill_betweenx(elevations[:-1], bulk_t_ice+bulk_t_ice*0.125, bulk_t_ice-bulk_t_ice*0.125,
                 zorder=1, color=c1, alpha=0.2)
ax4.plot(bulk_t_snowline, elevations[:-1], color=c2, lw=2, zorder=2, alpha=0.8, label='Snowline')
ax4.fill_betweenx(elevations[:-1], bulk_t_snowline+bulk_t_snowline*0.125, 
                  bulk_t_snowline-bulk_t_snowline*0.125,
                 zorder=1, color=c2, alpha=0.2)
ax4.plot(bulk_t_snow, elevations[:-1], color=c3, lw=2, zorder=2, alpha=0.8, label='Snow')
ax4.fill_betweenx(elevations[:-1], bulk_t_snow+bulk_t_snow*0.125, 
                  bulk_t_snow-bulk_t_snow*0.125,
...
ax5.fill_betweenx(elevations[:-1], coeffs['ice']+coeffs['ice']*0.21, 
                  coeffs['ice']-coeffs['ice']*0.21,
                  zorder=1, color=c1, alpha=0.2)
ax5.plot(coeffs['snowline'], elevations[:-1], color=c2, 
         lw=2, zorder=2, alpha=0.8, label='Snowline')
ax5.fill_betweenx(elevations[:-1], coeffs['snowline']+coeffs['snowline']*0.21, 
                  coeffs['snowline']-coeffs['snowline']*0.21,
                  zorder=1, color=c2, alpha=0.2)
ax5.plot(coeffs['snow'], elevations[:-1], color=c3, 
         lw=2, zorder=2, alpha=0.8, label='Snow')
ax5.fill_betweenx(elevations[:-1], coeffs['snow']+coeffs['snow']*0.21, 
                  coeffs['snow']-coeffs['snow']*0.21,
                  zorder=1, color=c3, alpha=0.2)
...
ax6.plot(coeffs['ice_gt'], elevations[:-1], color=c1, zorder=2, 
         lw=2, alpha=0.8, label='Glacier ice')
ax6.fill_betweenx(elevations[:-1], coeffs['ice_gt']+coeffs['ice_gt']*0.21, 
                  coeffs['ice_gt']-coeffs['ice_gt']*0.21,
                  zorder=1, color=c1, alpha=0.2)
ax6.plot(coeffs['snowline_gt'], elevations[:-1], color=c2, 
         lw=2, zorder=2, alpha=0.8, label='Snowline')
ax6.fill_betweenx(elevations[:-1], coeffs['snowline_gt']+coeffs['snowline_gt']*0.21, 
                  coeffs['snowline_gt']-coeffs['snowline_gt']*0.21,
                  zorder=1, color=c2, alpha=0.2)
ax6.plot(coeffs['snow_gt'], elevations[:-1], color=c3, 
         lw=2, zorder=2, alpha=0.8, label='Snow')
ax6.fill_betweenx(elevations[:-1], coeffs['snow_gt']+coeffs['snow_gt']*0.21, 
                  coeffs['snow_gt']-coeffs['snow_gt']*0.21,
                  zorder=1, color=c3, alpha=0.2)
...
fig.savefig(savepath + 'surface-radiative-feedback.png', dpi=200)
...
print(np.nanstd(xr_stats_temp_sig[:,:,0].values[sig_mask]))
